...
public:
	...
	inline void multiply(const T);
	inline void divide(const T);
	inline T dot(const vect3<T>) const;	//+ 点乘
	inline void cross(const vect3<T>);	//+叉乘

	inline void negative();		// 逆向
	inline T length() const;	//长度
	inline T length2() const; //长度平方
	inline void unit();	// 归一化
	...
	inline T operator*(const vect3<T>) const; // dot product 点乘
	inline vect3<T> operator%(const vect3<T>) const; // cross product	叉乘
	...
	inline vect3<T> operator-() const; // negation 反向
	inline T operator!() const; // absolute value, length 矢量长度
	inline vect3<T> operator~() const; // unit vector		矢量归一化
