...
inline size_t findFiles(const String& sPath, const String& sExt, vector<String>& filePaths){
	...
	if (fs::is_directory(plugPath)){
		...
		{
			if (!fs::is_directory(*dir_itr) && (*dir_itr).extension() == sExt){
...
public:
		...
		for (; it != m_extPros.end(); ++it)
		{
			if (it->m_sExt == ext){
				...
				if (hExtDll != NULL){
					...
					readFunc = (ReadFileFunc)GetProcAddress(hExtDll, funcName);
#else
					readFunc = (ReadFileFunc)GetProcAddress(hExtDll, it->m_sFuncName);
#endif // UNICODE
					hDll = hExtDll;
					break;
				}
	...
	//@ 获取处理ext文件类型的注册信息
	//@ ext 为文件后缀名 (.txt  .pts )
	ExtProcess* findExtProcess(const String& ext) const {
		vector<ExtProcess>::iterator it = m_extPros.begin();
		for (; it != m_extPros.end(); ++it)
		{
			if (it->m_sExt == ext){
...
inline HINSTANCE& getReadExtFunc(const ExtProcess& extPro, ReadFileFunc& readFunc){
	HINSTANCE hExtDll = LoadLibrary(extPro.m_sDllPath.c_str());
	if (hExtDll != NULL){
#ifdef UNICODE
		string funcName;
		unicodeToGB2312(extPro.m_sFuncName, funcName);
		readFunc = (ReadFileFunc)GetProcAddress(hExtDll, funcName);
#else
		readFunc = (ReadFileFunc)GetProcAddress(hExtDll, extPro.m_sFuncName);
#endif // UNICODE
	}
	return hExtDll;
}
// 加载插件文件夹中的 Dll 进行注册
inline int loadExtDlls(const String& plugPath, ExtRegister& extReg){
	...
	if (findFiles(plugPath, _T(".dll"), dllPaths) != 0){
		for (vector<String>::iterator it = dllPaths.begin(); it != dllPaths.end(); ++it)
		{
			...
			if (hExtDll != NULL){
				RegisterExt registerExt = (RegisterExt)GetProcAddress(hExtDll, "RegistExt");
				if (registerExt != NULL){
